"""
🎯 Sniper Bot - Versão Final para Render
Solução definitiva para o problema de event loop usando Updater síncrono
"""
...
if FLASK_AVAILABLE:
    ...
    def home():
        return jsonify({
            ...
            'telegram_active': bot_state.telegram_active,
            'telegram_error': bot_state.telegram_error,
...
def run_telegram_bot_sync():
    """Executa o bot Telegram de forma completamente síncrona"""
    ...
    try:
        logger.info("🤖 Iniciando bot Telegram (modo síncrono)...")
        
        # Criar aplicação sem usar async
        from telegram.ext import Updater
        
        # Usar Updater (versão síncrona) em vez de Application (assíncrona)
        updater = Updater(token=config['TELEGRAM_TOKEN'], use_context=True)
        dispatcher = updater.dispatcher
        
        # Converter handlers assíncronos para síncronos
        def sync_start_cmd(update, context):
            """Versão síncrona do comando /start"""
            welcome_text = f"""
🎯 Sniper Bot Base Network v2.0
...
"""
        ...
        def sync_status_cmd(update, context):
            """Versão síncrona do comando /status"""
            uptime = datetime.now() - bot_state.stats['start_time']
...
💰 Carteira:
• ETH: {bot_state.wallet_balance['eth']:.6f} ETH
• WETH: {bot_state.wallet_balance['weth']:.6f} WETH
• Total: ~${(bot_state.wallet_balance['eth'] + bot_state.wallet_balance['weth']) * 2500:.2f} USD
...
"""
        ...
        # Adicionar handlers síncronos
        dispatcher.add_handler(CommandHandler("start", sync_start_cmd))
        dispatcher.add_handler(CommandHandler("status", sync_status_cmd))
        
        # Marcar como ativo
        bot_state.telegram_active = True
        bot_state.telegram_error = None
        
        # Iniciar polling síncrono
        updater.start_polling()
        logger.info("✅ Bot Telegram iniciado com sucesso (modo síncrono)")
        
        # Manter rodando
        updater.idle()
        
    except Exception as e:
        error_msg = f"Erro executando bot Telegram: {e}"
        logger.error(f"❌ {error_msg}")
        bot_state.telegram_active = False
        bot_state.telegram_error = error_msg
    finally:
        bot_state.telegram_active = False
...
def main():
    ...
    # Iniciar bot Telegram em thread separada (modo síncrono)
    if TELEGRAM_AVAILABLE and config.get('TELEGRAM_TOKEN'):
        telegram_thread = Thread(target=run_telegram_bot_sync, daemon=True)
